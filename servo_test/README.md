...
```bash
python3 servo_test/test_servo_rpi.py --pin 12
```

Проверка центра:

```bash
python3 servo_test/test_servo_rpi.py --pin 12 --mode center --center-angle 90
```

Проверка нескольких углов:

```bash
python3 servo_test/test_servo_rpi.py --pin 12 --mode angles --angles 0,45,90,135,180
```

Медленный прогон туда-обратно:

```bash
python3 servo_test/test_servo_rpi.py --pin 12 --mode sweep --step 10 --delay 1.0 --repeat 0
```

Если сервомотор упирается в край или дрожит, подберите диапазон импульсов:

```bash
python3 servo_test/test_servo_rpi.py --pin 12 --min-pulse-us 600 --max-pulse-us 2400
```
