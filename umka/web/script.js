...
async function requestKioskExit() {
  ...
  if (!token) {
    exitMessage.textContent = 'Экран открыт не через ярлык UMKA. Закройте Chromium через Alt+F4 или по SSH.';
    return;
  }
  exitConfirm.disabled = true;
  exitMessage.textContent = 'Закрываю экран...';
  try {
    const response = await fetch(`http://127.0.0.1:3001/exit?token=${encodeURIComponent(token)}`, {
      method: 'POST'
    });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
  } catch (error) {
    console.error('Не удалось закрыть kiosk:', error);
    exitMessage.textContent = 'Не удалось закрыть экран. Используйте Alt+F4 или SSH.';
...
function handleFaceTouch(e) {
  ...
  if (x < thirdX) {
    ...
    const dirs = ['eyes-escape-left', 'eyes-escape-right', 'eyes-escape-up', 'eyes-escape-down'];
...
function clearAnimationClasses() {
  const classes = [
    'animation-wink-left', 'animation-wink-right', 'animation-double-blink',
    'animation-look-left', 'animation-look-right', 'animation-look-up', 'animation-look-down',
    'animation-squint', 'animation-smile', 'animation-thoughtful', 'animation-sleepy',
    'animation-roll', 'animation-flash', 'animation-swing', 'animation-bounce',
    'animation-tilt', 'animation-shake', 'animation-pulse', 'animation-confused',
    'animation-curious', 'animation-excited', 'animation-scan', 'animation-blush', 'animation-focus'
  ];
...
function setEmotion(emotion) {
  ...
  faceContainer.classList.remove('emotion-idle', 'emotion-surprised', 'emotion-happy', 'emotion-sad');
...
function scheduleIdleAnimation() {
  ...
  idleAnimationTimer = setTimeout(() => {
    if (currentState && currentState.state === 'idle' && !isRandomAnimationActive && !isTouching) {
...
function updateUI(data, previousState) {
  ...
  const cameraText = data.camera?.ok ? 'камера готова' : `камера: ${data.camera?.message || 'ошибка'}`;
  const modelText = data.model?.ok ? 'модель готова' : `модель: ${data.model?.message || 'ошибка'}`;
  diagnostics.textContent = `${cameraText} · ${modelText} · режим: ${data.hardware?.driver || '—'}`;
  diagnostics.classList.toggle('has-error', !data.camera?.ok || !data.model?.ok || Boolean(data.error));
  const statuses = {
  ...
  systemStatus.textContent = statuses[data.controllerState] || 'Система запущена';
  document.querySelectorAll('.btn[data-answer]').forEach(btn => {
...
function showWaste(data) {
  ...
  document.querySelectorAll('[data-section]').forEach(el => { el.textContent = data.section || '—'; });
  document.querySelectorAll('[data-action]').forEach(el => { el.textContent = data.lastAction || 'распознано'; });
}
