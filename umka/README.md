...
Назначение секций меняется в [sorter/config.py](sorter/config.py), в словаре
`AppConfig.class_to_section`. Например, `"paper": "section_1"` отправляет бумагу
в первую секцию. Чтобы изменить бак для типа мусора, поменяйте соответствующее
значение `section_1`–`section_4`.
...
### Запуск по SSH из активированной среды

Если после подключения уже видно приглашение `(.venv) umka@raspberrypi:~/app $`,
каталог и среда настроены. Для запуска камеры, распознавания, моторов и
...
Журнал выводится в SSH-терминал, остановка — **Ctrl+C**. Запускайте только один
экземпляр UMKA: перед запуском остановите программу, запущенную ярлыком, либо
службу командой `sudo systemctl stop umka`, если она работает.

Чтобы программа продолжала работать после закрытия SSH, используйте **вместо
команды выше** фоновый запуск:

```bash
nohup python /home/umka/app/app.py --hardware pca9685 > umka-ssh.log 2>&1 < /dev/null &
echo "PID программы: $!"
```
...
Здесь **Ctrl+C** завершает только просмотр журнала. Для остановки фонового
процесса выполните `kill -TERM PID`, заменив `PID` номером, выведенным при запуске.
Фоновый запуск оболочкой может отключать обработку `INT`, поэтому для него
используется `TERM`. Если номер потерян, найдите его командой:
...
- На рабочем столе Raspberry Pi нажмите **Открыть экран UMKA**. Повторно нажимать
  **Запустить UMKA** не нужно: программа уже запущена по SSH.
- На другом компьютере в той же сети откройте `http://<IP_RASPBERRY_PI>:3000`.

После принятия ответа «Да»/«Нет» галочка или крестик показывается на 0,8 секунды,
затем виден экран «Завершаю сортировку…». Карточка вопроса для этого предмета
больше не показывается. Система переходит к ожиданию нового предмета после
завершения механического цикла и подтверждённого освобождения площадки
(`clear_frames`).

Эмоция глаз после ответа длится 4 секунды, затем возвращается обычное выражение,
даже если сортировка ещё завершается. Длительность задаёт `FEEDBACK_EMOTION_MS`
в `web/script.js`. Опускание грустных лап задаёт `--sad-paw-drop` в `web/style.css`
(сейчас `12px`).

### Углы поворота и наклона для сброса

На Raspberry Pi редактируйте `~/app/sorter/hardware.py`. В проекте это файл
[sorter/hardware.py](sorter/hardware.py). Все углы задаются в начале файла,
перед классами драйверов. Таблица `ACTIONS` общая для GPIO и PCA9685:
...
- `rotate` — угол поворота к секции в градусах; по умолчанию канал PCA9685 `0`.
- `tilt` — угол наклона для сброса в эту секцию в градусах; по умолчанию канал `1`.

Например, в строке `section_1` измените `rotate`, чтобы настроить направление
поворота к первому баку, и `tilt`, чтобы настроить наклон при сбросе в него.

Нейтральные («нулевые») углы задаются рядом с таблицей `ACTIONS`:

```python
CENTER_TILT_ANGLE = 85    # Исходное положение наклона
CENTER_ROTATE_ANGLE = 50  # Исходное положение поворота
```

Последовательность движения: поворот к секции → наклон для сброса → возврат
наклона на `CENTER_TILT_ANGLE` → возврат поворота на `CENTER_ROTATE_ANGLE`.
Оба метода `center()` используют эти общие переменные; редактировать методы
драйверов для калибровки не нужно. Это абсолютные углы сервоприводов в градусах.

Назначение типов мусора секциям задаётся в `~/app/sorter/config.py`, в словаре
`class_to_section`, описанном выше. После изменения углов или назначения секций
перезапустите UMKA, чтобы применить настройки.

### Проверка сброса по секциям из консоли

Для подбора углов используйте [calibrate.py](calibrate.py). Он управляет моторами
через тот же драйвер и таблицу `ACTIONS`, что и UMKA, без запуска камеры,
распознавания и веб-сервера.

1. Остановите работающую UMKA. Если она запущена службой, выполните
   `sudo systemctl stop umka`.
2. Измените нужные `rotate` и `tilt` в `sorter/hardware.py` и сохраните файл.
3. В SSH-терминале с приглашением `(.venv) umka@raspberrypi:~/app $` запустите:

```bash
python calibrate.py --hardware pca9685
```

Скрипт покажет загруженные углы центра и секций и сразу вернёт механизм в центр. Затем вводите
команды по одной, нажимая **Enter**:

| Команда | Действие |
| --- | --- |
| `1` | Сброс в `section_1` и возврат в центр |
| `2` | Сброс в `section_2` и возврат в центр |
| `3` | Сброс в `section_3` и возврат в центр |
| `4` | Сброс в `section_4` и возврат в центр |
| `c` | Вернуть механизм в центр |
| `q` | Завершить проверку и освободить драйвер |

Например, введите `1`, дождитесь окончания движения, затем проверьте `2`, `3`
и `4`. Для нового подбора углов выйдите через `q`, измените файл и запустите
скрипт снова: изменения файла не подхватываются во время работы.

**Ctrl+C** тоже завершает проверку и освобождает драйвер. При выходе PCA9685
отключает PWM обоих моторов; прерванное движение автоматически не продолжается.

Без оборудования можно проверить команды в режиме имитации:

```bash
python calibrate.py --hardware console
```

Параметры подключения `--pca9685-address`, `--tilt-channel` и `--rotate-channel`
поддерживаются так же, как в `app.py`. По умолчанию наклон использует канал `1`,
поворот — канал `0`.
...
Чтобы открыть **Отладочное окно UMKA**, пять раз быстро коснитесь анимированного
логотипа. Касания должны уложиться в четыре секунды. В окне показаны состояние
камеры, модели и текущий режим работы; диагностика обновляется автоматически.
Кнопка **Вернуться** закрывает отладочное окно.

Для выхода из kiosk в этом же окне нажмите **Закрыть экран**. Закроется только
Chromium, а `umka.service`, камера и сортировка продолжат работать. Закрытие
Chromium через эту кнопку доступно при запуске через подготовленный
`umka-screen.desktop`; просматривать диагностику можно и в обычном браузере.
...
Проверки переходов экрана после ответа «Да»/«Нет» запускаются через Node.js
(нужен только для этих тестов, дополнительные npm-пакеты не требуются):

```bash
node --test tests/test_feedback_ui.cjs
```
